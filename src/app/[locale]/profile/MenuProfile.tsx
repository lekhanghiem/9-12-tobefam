...
const Dashboard = ({ children }: Readonly<{ children: React.ReactNode }>) => {
  ...
  return (
    <div className="flex h-full bg-[#f5f5f5]"> {/* Set parent div to full height */}
      <aside className={`h-screen bg-white ${isOpen === true ? 'w-[300px] ' : 'w-[70px] hidden absolute'} transition-all sticky top-0  sm:block`}>
        <div className="flex items-center justify-between w-full px-4 py-5 rounded-lg transition">
          <div className="flex items-center px-5">
            <IconButton
              ...
              sx={{ display: { sm: 'block' } }}
            >
              {isOpen === true ? <MdOutlineMenuOpen style={{ fontSize: '25px' }} /> : <MdOutlineMenu style={{ fontSize: '25px' }} />}
            </IconButton>
            <span className={`text-4xl font-bold ${isOpen === true ? 'opacity-100' : 'opacity-0'}`}>
              <Link href="profile">Profile</Link>
              <span className={`text-sm text-gray-400 pt-5 ${isOpen === true ? 'opacity-100' : 'opacity-0'}`}>v.01</span>
            </span>
          </div>
        </div>
    ...
    <main className={`flex-1 ${isOpen ? 'px-5' : 'sm:px-10 px-0'} h-full`}>
        <div className='w-full h-full bg-white shadow-md rounded-xl'>
          <div className='w-full h-[400px] relative'>
            <div className={`flex items-center px-5 absolute  ${isOpen ? 'hidden' : 'flex'}  sm:hidden`}>
            <IconButton
              color="inherit"
              aria-label="open drawer"
              edge="start"
            ...
            <div className='absolute bottom-20 right-20'>
              <Button color="secondary" sx={{ border: '1px solid #A303A0', bgcolor: 'white' }}>
                Edit Cover Photo
              </Button>
            </div>
